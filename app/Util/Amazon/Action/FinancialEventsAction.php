...
class FinancialEventsAction implements ActionInterface
{
    ...
    public function __construct(int $merchant_id, int $merchant_store_id, FinancialEvents $financialEvents)
    {
        ...
        $this->eventList = [
            // 配送
            ShipmentEventList::class => $financialEvents->getShipmentEventList(), // 亚马逊订单交易，包含亚马逊的订单收入及订单费用
            // 一个关于货物结算财务事件的信息列表
            ShipmentSettleEventList::class => $financialEvents->getShipmentSettleEventList(),
            // 退款
            RefundEventList::class => $financialEvents->getRefundEventList(), // 亚马逊订单退款，包含亚马逊的订单退款及订单退款
            // 一个网络混合交易事件
            GuaranteeClaimEventList::class => $financialEvents->getGuaranteeClaimEventList(),
            // 拒付
            ChargebackEventList::class => $financialEvents->getChargebackEventList(), // 买家信用卡拒付
            // 亚马逊支付
            PayWithAmazonEventList::class => $financialEvents->getPayWithAmazonEventList(), // 用户使用亚马逊账户绑定第三方平台进行收款，亚马逊会提供收款服务在此服务中扣除手续费
            ServiceProviderCreditEventList::class => $financialEvents->getServiceProviderCreditEventList(),
            // 赔偿撤销
            RetroChargeEventList::class => $financialEvents->getRetrochargeEventList(), // 分两种 order撤销和赔偿撤销。属于已经赔偿后重新撤销的金额，目前此事件含税费
            RentalTransactionEventList::class => $financialEvents->getRentalTransactionEventList(),
            // 广告
            ProductAdsPaymentEventList::class => $financialEvents->getProductAdsPaymentEventList(), // CPC广告服务，若卖家的广告服务使用卖家账户扣款则在此服务中结算，若选择信用卡支付则不在此服务中结算
            // 服务费
            ServiceFeeEventList::class => $financialEvents->getServiceFeeEventList(), // 亚马逊店铺或账号维度的服务费，主要包括订阅费、促销费等
            // 秒杀费用
            SellerDealPaymentEventList::class => $financialEvents->getSellerDealPaymentEventList(), // 亚马逊Lightning Deal Fee费用
            // 信用卡扣款
            DebtRecoveryEventList::class => $financialEvents->getDebtRecoveryEventList(), // 当用户应收金额不足以支付账单费用，在此类型中执行信用卡扣款业务，此类型不含广告费直接信用扣款
            // 贷款服务事件的列表
            LoanServicingEventList::class => $financialEvents->getLoanServicingEventList(),
            // 对卖方帐户的调整
            AdjustmentEventList::class => $financialEvents->getAdjustmentEventList(), // 亚马逊库存赔偿、亚马逊费用调整及预留金额
            // 一个SAFETReimbursementEvents.的列表
            SAFETReimbursementEventList::class => $financialEvents->getSafetReimbursementEventList(),
            // 早期评论人计划
            SellerReviewEnrollmentPaymentEventList::class => $financialEvents->getSellerReviewEnrollmentPaymentEventList(), // 亚马逊早期评论人计划扣款
            // 亚马逊库存清算服务
            FbaLiquidationEventList::class => $financialEvents->getFbaLiquidationEventList(), // 移除中类型为清算的订单，费用在此类型中结算
            // 优惠券手续费
            CouponPaymentEventList::class => $financialEvents->getCouponPaymentEventList(), // 产生促销订单后扣除优惠券的手续费用，0.06美元（美国）或60日元（日本）
            // 与亚马逊成像服务相关的收费事件列表
            ...
            // 清算调整费用
            RemovalShipmentAdjustmentEventList::class => $financialEvents->getRemovalShipmentAdjustmentEventList(), // 对应summary中Liquidation Adjustment
            // 关于试运财务事件的信息列表
...
//            ValueAddedServiceChargeEventList::class => $financialEvents->getValueAddedServiceChargeEventList(),
    ...
    public function run(): bool
    {
        ...
        foreach ($this->eventList as $eventName => $financialEventList) {
...
//            }
        ...
        foreach ($this->eventObjectList as $eventName => $eventObject) {
