...
class GetCatalogItem extends HyperfCommand
{
    ...
    public function configure(): void
    {
        ...
        $this->addArgument('merchant_id', InputArgument::REQUIRED, '商户id')
            ...
            ->addArgument('asin', InputArgument::REQUIRED, 'ASIN')
            ->addArgument('country_code', InputArgument::REQUIRED, '国家')
    ...
    public function handle(): void
    {
        ...
        $asin = $this->input->getArgument('asin');
        $country_code = $this->input->getArgument('country_code');

        AmazonApp::tok2($merchant_id, $merchant_store_id, $region, static function (AmazonSDK $amazonSDK, int $merchant_id, int $merchant_store_id, SellingPartnerSDK $sdk, AccessToken $accessToken, string $region, array $marketplace_ids) use ($asin, $country_code) {
            $console = ApplicationContext::getContainer()->get(StdoutLoggerInterface::class);
            $logger = ApplicationContext::getContainer()->get(AmazonCatalogLog::class);

            $included_data = ['classifications',  'images', 'productTypes', 'relationships', 'salesRanks', 'summaries'];
            $locale = null;
